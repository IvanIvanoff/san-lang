defmodule SanLang.Interpreter do
  ...
  def eval(
        {:lambda_fn_call, {:lambda_fn, args_names, _body} = lambda, {:list, _} = args_values},
        env
      ) do
  ...
  # def eval({:lambda_fn_call, {:lambda_fn_call, _args, _name} = inner_lambda, args}, env) do
  #   IO.inspect("INNER LAMBDA: #{inspect(inner_lambda)}")
  #   IO.inspect("ARGS: #{inspect(args)}")
  # end
